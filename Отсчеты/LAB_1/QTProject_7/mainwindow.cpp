...
void MainWindow::func_X()
{
    ...
    if(ui->spinBox_1->value() > 0)
    {
        ...
        double vecT[4] = {1, 0.4, 0.3}; // // after - после

        double vecS[2] = {0.2351, 1}; //Segment
    ...
    if(arg1 >= 0){
        ...
        for(size_t i = 0; i <= arg1*5; i+=5){
             ...
             //////////////////////
                  ...
                  switch(arg1 - i/5){
                  case(0):
                       elem.qComboBox_a->setCurrentIndex(2);
                      break;
                  case(5):
             ...
             QFont newFont("Times New Roman", 12, QFont::Times, true);
              ...
              if(i < arg1*5){
                  elem.qLabel->setText("<html><head/><body><p>x<span style=\" vertical-align:super;\">"+ QString::number(arg1 - i/5)+"</span></p></body></html>");
              }else{
                  elem.qLabel->setText("<html><head/><body><p>x<span style=\" vertical-align:super;\">"+ QString::number(arg1 - i/5)+"</span></p></body></html>");
              }
...
void MainWindow::on_pushButton_2_clicked()
{
    ...
    if(qElem_s_1.count() > 0){
        ...
        for(size_t i = 0; i <= ORD; i++){
                ...
                C_a[i] = qElem_s_1[i].qLineEdit_a->text().toDouble();
        }
        strHtml_A.append("</tr></table>");
    }
    ...
    auto f = [&](double x) -> double{
        ...
        auto C_a_fun = [&](int index, int i, double buff) -> double {
         ...
         C_f_fun = [&](int index, int i) -> double {//auto
            ...
            case(3):
                if((x == 0) && (ORD - i) == 0){
                    return A_a[i]*1*C_a_fun(C_a[i], i, 0.0);
                }else{
                    return A_a[i]*qPow(x, ORD - i)*C_a_fun(C_a[i], i, 0.0);
                }
            case(0):
                //double X = A_a[i]*qPow(x, ORD - i)*C_a_fun(C_a[i], i);
                if((x == 0) && (ORD - i) == 0){
                    return qTan( A_a[i] * 1 * C_a_fun( C_a[i], i, ( A_a[i] * qPow(x, ORD - i) )));// m *(1 + (... / (buff = m))) == (m + ...);
                }else{
                    return qTan( A_a[i] * qPow(x, ORD - i) * C_a_fun( C_a[i], i, ( A_a[i] * qPow(x, ORD - i) )));
                }
            case(1):
                if((x == 0) && (ORD - i) == 0){
                    return std::log10( A_a[i] * 1 * C_a_fun( C_a[i], i, ( A_a[i] * qPow(x, ORD - i) )));// m *(1 + (... / (buff = m))) == (m + ...);
                }else{
                    return std::log10( A_a[i] * qPow(x, ORD - i) * C_a_fun( C_a[i], i, ( A_a[i] * qPow(x, ORD - i) )));
                }
            case(2):
               return 1;
            default:
    ...
    auto F = [&](double x, double b) -> double {
