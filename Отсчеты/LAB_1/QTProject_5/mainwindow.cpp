...
void MainWindow::func_X()
{
    ...
    {
        double vecF[4] = {1, -3, 6, -2}; // f(x)
        double vecS[2] = {0, 2}; //Segment
        double eps = 0.001;
    ...
    if(arg1 >= 0){
        ...
        for(size_t i = 0; i <= arg1*2; i+=2){
             ...
             QFont newFont("Times New Roman", 12, QFont::Times, true);
              ...
              if(i < arg1*2){
                  elem.qLabel->setText("<html><head/><body><p>x<span style=\" vertical-align:super;\">"
                                       + QString::number(arg1 - i/2)+"</span>+</p></body></html>");
              }else{
                  elem.qLabel->setText("<html><head/><body><p>x<span style=\" vertical-align:super;\">"
                                       + QString::number(arg1 - i/2)+"</span></p></body></html>");
              }
...
void MainWindow::on_pushButton_2_clicked()
{
    ...
    ui->textBrowser_1->setHtml(ui->textBrowser_1->toHtml() + "<b>Точность расчетов:</b> ε = "
                               + Epsilon.qLineEdit->text() + ";" + "<hr>");
    ...
    //d_2 - derivative 1 -  Вторая производная:
    ...
    auto s = [&,ORD = ORD,A = A](double xj, size_t i = 1) -> double {
        return ( xj - ( f(xj) / f_d_1(xj) ) );
    };
    ...
    if(f(B[0])*f_d_2(B[0]) > 0){
         x_0 = B[0];
    }else if (f(B[1])*f_d_2(B[1]) > 0){
         x_0 = B[1];
    }else{
        qDebug() << "Отрезок отделения указан неверно!";
        exit(0);
    }
    ...
    do{
        ...
        ui->textBrowser_1->setHtml(ui->textBrowser_1->toHtml() + "<b>x_j = </b>"
                                   + QString::number(x_j, 'f', Epsilon.qLineEdit->text().length() - 2) + " ;");
