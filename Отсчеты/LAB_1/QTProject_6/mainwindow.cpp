...
void MainWindow::func_X()
{
    ...
    {
        double vecF[4] = {1, -3, -24, -8}; // f(x)

        double vecS[2] = {6, 7}; //Segment
...
void MainWindow::on_pushButton_2_clicked()
{
    ...
    auto f = [A = A, ORD = ORD](double x) -> double {
    ...
    //d_1 - derivative 1 -  Первая производная:
    auto f_d_1 = [A = A, ORD = ORD](double x) -> double {
            double xi = 0;

        for(size_t i = 0; i < ORD; i++){
            xi += (ORD - i)* A[i] * qPow(x, (ORD - i) - 1);
        }

        return xi;
    };

    //d_2 - derivative 1 -  Первая производная:
    auto f_d_2 = [A = A, ORD = ORD](double x) -> double {
            double xi = 0;

        for(size_t i = 0; i < ORD; i++){
            xi += ((ORD - i) - 1)*(ORD - i)* A[i] * qPow(x, (ORD - i) - 2);
        }

        return xi;
    };

    auto s = [&](double xj, double _xj) -> double {
        return ( xj - ( f(xj) / (f(_xj) - f(xj)) ) * (_xj - xj) );
    };

    auto _s = [&](double _xj) -> double {
        return ( _xj - ( f(_xj) / f_d_1(_xj) ) );
    };
    ...
    double x_i(0), x_j(0), _x_i(0), _x_j(0);
    ...
    do{
        ...
        ui->textBrowser_1->setHtml(ui->textBrowser_1->toHtml() + "<b>_x_j = </b>" +  QString::number(_x_j, 'f', Epsilon.qLineEdit->text().length() - 2) + " ;");
        ui->textBrowser_1->setHtml(ui->textBrowser_1->toHtml() + "<b>Номер итерации: </b>" +  QString::number(count, 'd', 0) + " ;");

    }while(qFabs(_x_j - x_j) >= eps);

    double csi = (x_j + _x_j) / 2;

    ui->textBrowser_1->setHtml(ui->textBrowser_1->toHtml() +"<hr>" + "<b>Ответ: </b><b>ξ = </b> " +  QString::number(csi, 'f', Epsilon.qLineEdit->text().length() - 2) + " ;");
    ui->textBrowser_1->setHtml(ui->textBrowser_1->toHtml() + "<b>Номер итерации: </b>" +  QString::number(count, 'd', 0) + " ;");
    ////////////////////////////////////////////////
